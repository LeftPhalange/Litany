...
type onClick = (title: string, description: string, priority: TaskPriority, id?: number) => void;

/* Generic task dialog shared between AddTask and EditTask */

export default function TaskDialog({ task, title, onClick, closeDialog, open }: { task: Task, title: string, onClick: onClick, closeDialog: () => void, open: boolean }) {
            ...
            <div className="flex flex-row space-x-2 pt-2 w-full">
                <Button onClick={() => {
                    ...
                    if (taskTitle.length > maxTitleLength) {
                        ...
                        if (task.taskId == -1) {
                            onClick(taskTitle, taskDescription, taskPriority);
                        } else {
                            onClick(taskTitle, taskDescription, taskPriority, task.taskId);
                        }
                        closeDialog();
                    }
                }} className="bg-neutral-800 hover:bg-neutral-700 border border-1 border-neutral-600 rounded-lg py-2 px-8 font-semibold text-xs">{ task.taskId <= -1 ? "Create" : "Update" }</Button>
